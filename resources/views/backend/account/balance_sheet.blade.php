...
<script type="text/javascript">
    ...
    function datatable_sum(dt, is_calling_first) {
        ...
        let credit = dt.cells(rows, 3, { page: 'current' }).data().sum();
        let debit  = dt.cells(rows, 4, { page: 'current' }).data().sum();
        let balance = dt.cells(rows, 5, { page: 'current' }).data().sum();

        $(dt.column(3).footer()).html(formatNumber(credit));
        $(dt.column(4).footer()).html(formatNumber(debit));
        $(dt.column(5).footer()).html(formatNumber(balance));
    }
